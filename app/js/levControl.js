//Mousefunctions etc
    ...
    function dragSingleBall(ball){
      ...
      if(ball.position.y < tempY){
        ball.material = new THREE.MeshLambertMaterial({    color: 0x00FFFF   });
      }

      if(ball.position.y > tempY){
        ball.material = new THREE.MeshLambertMaterial({    color: 0x3399FF    });
      }
            ...
            if (ball.id == crossA1[i]){
              //edge1 case
                if (in_list(ball_in_plane.id, crossA1)){
                  ball_in_plane.position.y = ball.position.y;
                  if (ball_in_plane.position.y < 0) {ball_in_plane.position.y = 0}
                }
    ...
    function dragColumn(ball){
      ...
      for (var i = 0; i < 5; i++) {
        if (column[i].position.y < 0) {column[i].position.y = 0}
